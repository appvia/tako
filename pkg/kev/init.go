...
func printInitProjectWithOptionsError(ui komando.UI) {
	ui.Output("")
	ui.Output("Project had errors during initialisation.\n"+
		...
		komando.WithErrorBoldStyle(),
		komando.WithIndentChar(komando.ErrorIndentChar),
	)
}

func printInitProjectWithOptionsSuccess(ui komando.UI, envs Environments) {
	ui.Output("")
	ui.Output("Project initialised!", komando.WithStyle(komando.SuccessBoldStyle))
	ui.Output(fmt.Sprintf("A '%s' file was created. Do not edit this file.\n", ManifestFilename)+
		...
		komando.WithStyle(komando.SuccessStyle),
	)
	var namedValues []komando.NamedValue
	for _, env := range envs {
		namedValues = append(namedValues, komando.NamedValue{Name: env.Name, Value: env.File})
	}
	ui.Output("")
	ui.Output("And, the following deployment env files have been created:", komando.WithStyle(komando.SuccessStyle))
	ui.NamedValues(namedValues, komando.WithStyle(komando.SuccessStyle))
	ui.Output("")
	ui.Output("Update these to configure your deployments per related environment.", komando.WithStyle(komando.SuccessStyle))
	ui.Output("")
