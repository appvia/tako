...
import (
	...
	kustomizeTypes "github.com/kubernetes-sigs/kustomize/pkg/types"
...
func PrintList(objects []runtime.Object, opt ConvertOptions, rendered map[string][]byte) error {
	...
	k8sOpt := &kubernetes.ConvertOptions{
		CreateChart:  opt.CreateChart,
		ToStdout:     opt.ToStdout,
		GenerateJSON: opt.GenerateJSON,
		EmptyVols:    opt.EmptyVols,
		Volumes:      opt.Volumes,
		InputFiles:   opt.InputFiles,
		OutFile:      opt.OutFile,
		YAMLIndent:   opt.YAMLIndent,
	}

	err := kubernetes.PrintList(objects, *k8sOpt, rendered)
	if err != nil {
		return err
	}
	...
	for fileName := range rendered {
		kustomization.Resources = append(kustomization.Resources, path.Base(fileName))
	}
	...
	ioutil.WriteFile(path.Join(opt.OutFile, "kustomization.yaml"), kBytes, 0644)
